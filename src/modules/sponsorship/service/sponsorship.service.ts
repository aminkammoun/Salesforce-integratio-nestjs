...
export class SponsorshipService {
    ...
    async findSpFromSalesforceByWordpressId(wordpressid: string) {
        try {
            ...
            Child__r.Age_Calculated__c, 
            Child__r.Gender__c,
            Child__r.Next_Birthday__c, 
            Child__r.First_Name__c, 
            Child__r.Last_Name__c,
            Child__r.Profile_Picture_Image__c, 
            Status__c, 
            Donor__c, 
            Donor__r.Word_Press_Id__c 
            FROM Sponsorship__c  WHERE Donor__c= '${wordpressid}'`;
            const token = await authenticateSalesforce();
    ...
    async repaireSp(idsp: string, campaignId: string, source: string, nationality?: string, territory?: string) {
        ...
        for (const sponsorship of sponsorships) {
            const recurring = await this.RecurringModel.create({
                ...
                npe03__Recurring_Donation_Campaign__c: campaignId || '',
